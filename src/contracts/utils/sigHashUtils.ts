...
export type PrevoutsCtx = {
    ...
    preOutputIndexBytes: ByteString
}
...
export class SigHashUtils extends SmartContractLib {
    ...
    static checkPrevoutsCtx(
        ...
        // check input index
        assert(TxUtil.checkIndex(prevoutsCtx.inputIndex, inputNumber))
        // check vout
        assert(
            TxUtil.checkIndex(
                prevoutsCtx.outputIndex,
                prevoutsCtx.preOutputIndexBytes
            )
        )
        // check prevout
        assert(
            prevoutsCtx.prevouts[Number(prevoutsCtx.inputIndex)] ==
                prevoutsCtx.preTxhash + prevoutsCtx.preOutputIndexBytes
